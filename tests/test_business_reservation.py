...
def test_csv_repository_initialization_append_and_update(tmp_path) -> None:
...
def test_quota_exceeded(tmp_path) -> None:
    ...
    assert "名额已满" in str(result["message"])


def test_reservation_date_outside_next_seven_days_fails(tmp_path) -> None:
    service = ReservationService(tmp_path)
    result = service.create_reservation(make_request(date.today() + timedelta(days=8)))

    assert result["success"] is False
    assert "未来7天内" in str(result["message"])


def test_same_plate_number_cannot_reserve_twice_same_day(tmp_path) -> None:
    service = ReservationService(tmp_path)
    day = date.today() + timedelta(days=1)
    ...
    assert result["success"] is False
    assert "同一车牌" in str(result["message"])


def test_disabled_campus_fails(tmp_path) -> None:
    service = ReservationService(tmp_path)
...
def test_cancellation_releases_quota(tmp_path) -> None:
    service = ReservationService(tmp_path)
    day = date.today() + timedelta(days=1)
    service.repository.update(
        "campus_configs.csv",
    ...
    cancel = service.cancel_reservation(str(first["reservation_id"]))
